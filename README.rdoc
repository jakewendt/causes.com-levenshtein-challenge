...
grep -vs -E '^(.one|one.|o.e|.ne|on.)$' levenshtein.list > newlevenshtein.list	### INCOMPLETE REGEX
...
create regex that is a list of each letter replaced with a .?, plus a leading . and trailing .
(could be leading and trailing .?, but the given word won't be there so pointless)

convert "causes" into ".causes|.?auses|c.?uses|ca.?ses|cau.?es|caus.?s|cause.?|causes."

grep -E '^(.causes|.?auses|c.?uses|ca.?ses|cau.?es|caus.?s|cause.?|causes.)$' indifferents > newfriends
grep -vs -E '^(.causes|.?auses|c.?uses|ca.?ses|cau.?es|caus.?s|cause.?|causes.)$' indifferents > newindifferents


#	seed it
echo "causes" > new_friends.list

Loop over newfriends...

	append each newfriend to checked_friends.list
	append search results to newfriends
	remove newfriends from indifferents
